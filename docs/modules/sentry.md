...
- `watcher.py` — `SentryWatcher`: watchdog-based OS hooks emitting `FileChanged/Created/Deleted/Moved` signals; ignores noise (`.git`, dot-directories, `__pycache__`, `*.db`, `*.lock`, …). Every event is attributed to its owning registered project (longest path prefix) and bumps that project's `last_activity`, keeping stage-confidence decay live. Auto-starts on server boot when `DCP_WATCH_ROOTS` (comma-separated) is set; roots refresh automatically as discovery registers new projects.
